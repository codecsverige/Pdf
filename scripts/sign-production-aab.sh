...
for name in KEYSTORE_PATH STORE_PASSWORD KEY_ALIAS KEY_PASSWORD; do
  test -n "${!name}" || { echo "Missing required signing value: $name" >&2; exit 2; }
done

test -s "$INPUT_AAB"
test -s "$KEYSTORE_PATH"

ACTUAL_SHA1="$(keytool -list -v -keystore "$KEYSTORE_PATH" -storepass "$STORE_PASSWORD" -alias "$KEY_ALIAS" 2>/dev/null | sed -n 's/^[[:space:]]*SHA1: //p' | head -n 1)"
if [[ "$ACTUAL_SHA1" != "$EXPECTED_SHA1" ]]; then
  echo "REFUSING TO SIGN PDF Pro with an unregistered key." >&2
...
jarsigner -keystore "$KEYSTORE_PATH" -storepass "$STORE_PASSWORD" -keypass "$KEY_PASSWORD" -sigalg SHA256withRSA -digestalg SHA-256 "$OUTPUT_AAB" "$KEY_ALIAS"
...
if [[ "$SIGNED_SHA1" != "$EXPECTED_SHA1" ]]; then
  rm -f "$OUTPUT_AAB"
  echo "Signed AAB certificate mismatch; output removed." >&2
  exit 1
fi

printf 'PDF Pro signing verified.\nPackage: %s\nCertificate SHA1: %s\nOutput: %s\n' "$EXPECTED_PACKAGE" "$SIGNED_SHA1" "$OUTPUT_AAB"
